...
MIGRATIONS = [
    ...
    ("0013_rule_proposals", "CREATE TABLE IF NOT EXISTS rule_proposals (id integer primary key, proposal_key varchar(64) NOT NULL, rule_version varchar(32) NOT NULL DEFAULT 'rules-v6', revision integer NOT NULL DEFAULT 1, target_category varchar(48) NOT NULL, matcher json NOT NULL DEFAULT '{}'::json, evidence json NOT NULL DEFAULT '[]'::json, source_device_ids json NOT NULL DEFAULT '[]'::json, support_count integer NOT NULL DEFAULT 1, status varchar(24) NOT NULL DEFAULT 'open', created_by varchar(120) NOT NULL, reviewed_by varchar(120), review_note text, reviewed_at timestamp, created_at timestamp NOT NULL DEFAULT now()); CREATE INDEX IF NOT EXISTS ix_rule_proposals_key ON rule_proposals(proposal_key); CREATE INDEX IF NOT EXISTS ix_rule_proposals_status ON rule_proposals(status)"),
]
